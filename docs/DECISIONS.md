...
- **Rejected:** treating marker-only as live (false positive after id rename). Dropping Grok PostToolUse (black-box log and surprise-write freeze still need it; the TUI lines were 127 failures, not successful `{}` posts). Agent-written hook files.
